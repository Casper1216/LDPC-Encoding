...
bool is_DSS_2(vector<int>& s,set<int>& diff,set<int>& diff2,int v){
	set<int> diff_o;	//d1 diff
	set<int> diff_a;	//d1 diff2
	for(int i=0;i<s.size();i++){
		for(int j=0;j<s.size();j++){
			if(i!=j){
				int d = (int)(s[j]-s[i])%v;
				...
				if(!diff.count(d)&&!diff_o.count(d)){
					...
					return false;
				if(!diff2.count(d_2)&&!diff_a.count(d_2)){
					diff_a.insert(d_2);
...
void DSS(int v,int k,vector<int>& d1,vector<int>&d2){	//建立一個 (v,k,2) 的 DSS
	...
	while(d2.size()<k){
		...
		if(!is_DSS_2(d2,diff,diff2,v)){		//有重複數字 or 不是DSS 刪除並重新找數字 
